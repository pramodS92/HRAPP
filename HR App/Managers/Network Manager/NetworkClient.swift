...
class NetworkClient {
    ...
    func requestOtp(requestInfo: OTPRequestInfo, isSecure: Bool, completion: @escaping (Result<String>) -> Void) {
        // Check internet connection availability
        
        let networkSession: Alamofire.Session = isSecure ? manager : .default
        networkSession.request(requestInfo.url, method: requestInfo.method, parameters: requestInfo.params as? Parameters, encoding: JSONEncoding.default, headers: requestInfo.headers).responseJSON { response in
